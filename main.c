...
// note that Bob is a fictional person we created to test our code with, he was player1 and he won many times!

1. void insert(char wordToInsert[], wordNode* array[]):
    - requires: a character array (string) containing the word to be inserted and array of pointers to Wordnode 
                representing the head nodes of linked list for each character of the alphabet.
    - effects: Inserts a new node to the  linked list appropriately according to the first letter of the word.

2. char* botSpell(wordNode *head, int *count, const char level[], char *array[], int size):
    - requires: a pointer to the head of a linked list, an array of integers that represents the count of words 
                starting with each letter of the alphabet, a character array (string) "level" representing the 
                difficulty level (easy/hard), an array of strings that contains previously chosen words, and an 
                integer representing the size of the array.
    - effects: - If the level is "easy", Bot picks a word whose last letter has the maximum frequency in the list 
                 of letter counters, providing the opponent with more options to choose from later on. 
               - If the level is "hard", Bot picks a word ehose last letter has the minimum frequency in the list 
                 of letter counters, providing the opponent with less options to choose from later on.
               - In both cases, no word returned is one that was already chosen.

3. displayEffect():
    - requires: nothing 
    - effects:prints a dialogue that introduces the game to the player, with delays in between for better 
      readability.

4. void displayStart(char name1[], char name2[]):
    - requires: two character arrays (strings) representing the names of first (Bob!) and second player.
    - effects: prints a starting dialogue using the names of the players.

5. int wordsStartingWithChar(char *words[], char c, int sizeOfArray):
    - requires: an array of strings, a character "c", and the size of the array provided.
    - effects: check if there exist words in the array provided that start with the character "c".

6. void toLowerCase(char word[]):
    - requires: an array of characters (string).
    - effects: converts the given string to lowercase.

7. int isFound(char word[], char *array[], int size):
    - requires: a character array (string), an array of character arrays (strings), and an integer 
                representing its size.
    - effects: returns 1 if the character array (string) exists in the given array, and returns 0 otherwise.

8. int sameLetter(char* oldWord, char* newWord):
    - requires: two character arrays (strings).
    - effects: returns 1 if the last character in the first character array (string) is the same as the first 
               character in the second character array, otherwise it returns 0.

9. void printWords(char *words[], int sizeOfArray):
    - requires: an array of character arrays (strings), and an integer representing the size of the array.
