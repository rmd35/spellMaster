...
typedef struct wordNode_Marauders {
    char word[50];
    struct wordNode_Marauders *next;
} wordNode_Marauders;
...
void insert_Maruaders(char wordToInsert[], wordNode_Marauders *array[]) {
    int index = tolower(wordToInsert[0]) - 'a';
    wordNode_Marauders *newNode = malloc(sizeof(wordNode_Marauders));
...
int cast_spell_Marauders(char **spells, int num_spells, char *previous_spell, char **used_spells, int num_used_spells) {
    ...
    wordNode_Marauders *letteredwords[alphabet];
    for (int i = 0; i < alphabet; i++) {
    ...
    for (int i = 0; i < num_spells; i++) {
        if (!isFound_Marauders(spells[i], used_spells, num_used_spells)) {
            ...
            insert_Maruaders(spells[i], letteredwords);
        }
    }
    ...
    wordNode_Marauders *currentLinkedList = letteredwords[letterIndexForList];
