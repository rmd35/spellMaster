...
int main() {
    ...
    if (chosenWords[turnCount] == NULL) {
     ...
     if (!isFound(retaliate, words, wordCount) || 
        !wordsStartingWithChar(words, retaliate[strlen(retaliate) - 1], wordCount) || 
        (turnCount >= 2 && !sameLetter(chosenWords[turnCount - 2], retaliate)) ||
        isFound(retaliate, chosenWords, turnCount - 1)) {
        break;
    }
