...
int main() {
    ...
    if (chosenWords == NULL) {
        ...
        free(chosenWords); // cleans up previous memory allocation
        return 1;
    }
    int turnCount = 0;
    ...
    if (isFound(retaliate, chosenWords, turnCount - 1) == 1)
        ...
        printf("%s offend the laws of the duel! The characters do not match! Read for Merlin and Morgana's sake!", Playing);
