...
2. partitions char* botSpell(wordNode *head, int *count, const char level[], char *array[], int size) as follows:
   ...
   - Test Case 4: Ensure that the word it chooses is not one that is already chosen. In the case that all words with a certain letter are chosen
     spellBot will not be accessed, as one of the conditions in the do-while loop tackles this case.
...
4. partitions void displayStart(char name1[], char name2[]) as follows: 
   - Test Case 1: Verify that the function displays the starting dialogue correctly, with the names in the correct positions.
   - Test Case 2: Ensure that the countdown is displayed correctly.
   - Test Case 3: Check if the function handles the sleep function appropriately.
   - Test Case 4: Test with names whose lengths are the maximum lengths.
...
6. partitions void toLowerCase(char word[]) as follows: 
   ...
   - Test Case 2: Convert a string with all uppercase letters, and ensure that all letters are converted to lowercase.
   - Test Case 3: Convert a string with all lowercase letters, and ensure that all letters remain in lowercase.
   - Test Case 4: Convert an empty string, and ensure it remains the same
   - Test Case 5: Convert a string of no-letters, and ensure it remains the same.

7. int isFound(char word[], char *array[], int size) as follows: 
   - Test Case 1: Check if the function correctly identifies the existence of a word in the array provided and ensure that it returns 1.
   - Test Case 2: Check if the function recognizes the absence of a word in the array provided and ensure that it returns 0.

8. int sameLetter(char* oldWord, char* newWord) as follows: 
   - Test Case 1: Test with two words where the last letter of the first word matches the first letter of the second word and ensure that it returns 1.
   - Test Case 2: Test with two words where the last letter of the first word does not match the first letter of the second word and ensure that it returns 0.
   - Test Case 3: Test with an empty string, and ensure that 1 will be returned.
   - Test Case 4: Test with a string of length 1, and ensure that the result returned is either 1 or 0 accordingly.
...
10. partitions int main() as follows: 
   ...
   - Test Case 4: Test each losing condition, in both difficulty levels.
   - Test Case 5: Test with an empty file, annd ensure that the if-statements catch the error.
   - Test Case 6: Test with a file that does not maintain the file-format expected

11. rand():
   - Test Case 1: Ensure it is returning a different integer each time, within the range provided.
   - Test Case 2: Test with multiple ranges.
*/
